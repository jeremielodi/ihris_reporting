...
from manage.services.educational_level import level as educational_level
...
from manage.services.organization_level import level as organization_level
from manage.services.dashboard import dashboard
# NOTE: manage/services/dashboard_query/query.py is not mounted here on purpose.
# It imports `HippoDashboardQuery` from manage/models.py, which doesn't exist
# (no ORM model, no DB table/migration for it) - the module can't run as-is.
# Needs the model + migration built out before it can be wired in.
...
router.include_router(educational_level.apiRouter, tags=["educational_levels"])
...
router.include_router(payment_frequency.apiRouter, tags=["payment_frequencies"])
...
router.include_router(organization_level.apiRouter, tags=["OrganizationLevels"])
