...
class HippoInstitutionTypeBase(BaseModel):
...
class HippoInstitutionTypeCreate(HippoInstitutionTypeBase):
    id: Optional[str] = None
    created: Optional[datetime] = datetime.now()
    last_modified: Optional[datetime] = datetime.now()  # Default to now if not provided
    name:str

class HippoInstitutionTypeUpdate(HippoInstitutionTypeBase):
    last_modified: Optional[datetime] = datetime.now()  # Default to now if not provided


class HippoInstitutionTypeRead(HippoInstitutionTypeBase):
    id: str
