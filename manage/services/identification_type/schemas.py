...
class HippoIdentificationTypeBase(BaseModel):
...
class HippoIdentificationTypeCreate(HippoIdentificationTypeBase):
    id: Optional[str] = None
    created: Optional[datetime] = datetime.now()
    last_modified: Optional[datetime] = datetime.now()  # Default to now if not provided
    name:str

class HippoIdentificationTypeUpdate(HippoIdentificationTypeBase):
    last_modified: Optional[datetime] = datetime.now()  # Default to now if not provided


class HippoIdentificationTypeRead(HippoIdentificationTypeBase):
    id: str
