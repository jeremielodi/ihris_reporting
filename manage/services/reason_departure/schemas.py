...
class HippoReasonDepartureBase(BaseModel):
...
class HippoReasonDepartureCreate(HippoReasonDepartureBase):
    id: Optional[str] = None
    created: Optional[datetime] = datetime.now()
    last_modified: Optional[datetime] = datetime.now()  # Default to now if not provided
    name:str

class HippoReasonDepartureUpdate(HippoReasonDepartureBase):
    last_modified: Optional[datetime] = datetime.now()  # Default to now if not provided


class HippoReasonDepartureRead(HippoReasonDepartureBase):
    id: str
