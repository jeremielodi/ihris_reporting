...
class HippoPaymentFrequencyBase(BaseModel):
...
class HippoPaymentFrequencyCreate(HippoPaymentFrequencyBase):
    id: Optional[str] = None
    created: Optional[datetime] = datetime.now()
    last_modified: Optional[datetime] = datetime.now()  # Default to now if not provided
    name:str

class HippoPaymentFrequencyUpdate(HippoPaymentFrequencyBase):
    last_modified: Optional[datetime] = datetime.now()  # Default to now if not provided


class HippoPaymentFrequencyRead(HippoPaymentFrequencyBase):
    id: str
