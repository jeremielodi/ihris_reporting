...
export default defineComponent({
    ...
    emits: ['reloadIdentificationList'],
    setup() {},
    methods: {
        toggle(event) {
        ...
        closeDialog(result) {
            if (result) {
                this.$emit('reloadIdentificationList', true);
            }
            this.display = false;
                    ...
                    items: [
                        {
                            ...
                            command: () => {
                                this.$router.push(`/manage/identification_create?id=${this.entity.id}`);
                            }
                        }
                    ]
                }
...
<template>
    <div style="text-align: right" :data-testid="actionId" :id="actionId">
        ...
        <ConfirmModal :identification="entity" :close="DeleteConfirmDialog" :display="displayConfirm"> </ConfirmModal>
    </div>
</template>
