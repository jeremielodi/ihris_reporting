...
import FacilityAction from './actions.vue';
...
<template>
    <div class="card manage-container" style="height: 90vh">
        ...
        >
                ...
                <div class="flex justify-content-between flex-column sm:flex-row">
                    ...
                    <span>
                        <InputGroup>
                            ...
                            <Button :label="$t('FORM.BUTTONS.ADD')" @click="this.$router.push('/manage/facility_create')" icon="pi pi-plus"/>
                           </InputGroup>
                    </span>
                </div>
            </template>
            ...
            <Column field="location" :header="$t('TREE.HEALTH_AREA')" />
            <Column field="actions" :header="$t('Actions')" style="width: 80px;">
                <template #body="{ data }">
                    <FacilityAction :entity="data" action-id="${data.id}" />
                </template>
            </Column>
        </DataTable>
        <div>
            <b> {{ facilitys.length }} {{ $t('TREE.FACILITY') }} </b>
        </div>
    </div>
</template>
<style scoped>
