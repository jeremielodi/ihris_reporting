...
export default defineComponent({
    ...
    methods: {
        ...
        closeDialog(result) {
            if (result) {
                this.$emit('reloadDocumentTypeList', true);
            }
            this.display = false;
                    ...
                    items: [
                        {
                            ...
                            command: () => {
                                this.$router.push(`/manage/document_type_create?id=${this.entity.id}`);
                            }
                        }
                    ]
                }
...
<template>
    <div style="text-align: right" :data-testid="actionId" :id="actionId">
        ...
        <ConfirmModal :documenttype="entity" :close="DeleteConfirmDialog" :display="displayConfirm"> </ConfirmModal>
    </div>
</template>
