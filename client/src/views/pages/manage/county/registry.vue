...
import CountyAction from './actions.vue';
...
<template>
    <div class="card manage-container" style="height: 90vh">
        ...
        >
                ...
                <div class="flex justify-content-between flex-column sm:flex-row">
                    ...
                    <span>
                        <InputGroup>
                            ...
                            <Button :label="$t('FORM.BUTTONS.ADD')" @click="this.$router.push('/manage/county_create')" icon="pi pi-plus"/>
                           </InputGroup>
                    </span>
                </div>
            </template>
            ...
            <Column field="district" :header="$t('TREE.DISTRICT')" />
            <Column field="actions" :header="$t('Actions')" style="width: 80px;">
                <template #body="{ data }">
                    <CountyAction :entity="data" action-id="${data.id}" />
                </template>
            </Column>
        </DataTable>
        <div>
            <b> {{ countys.length }} {{ $t('TREE.COUNTY') }} </b>
        </div>
    </div>
</template>
<style scoped>
