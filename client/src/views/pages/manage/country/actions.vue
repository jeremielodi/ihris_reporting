...
export default defineComponent({
    ...
    methods: {
        ...
        closeDialog(result) {
            if (result) {
                this.$emit('reloadCountryList', true);
            }
            this.display = false;
        ...
        DeleteConfirmDialog(result) {
            ...
            CountryService.delete(this.entity.id)
                .then(() => {
                    ...
                    items: [
                        {
                            ...
                            command: () => {
                                this.$router.push(`/manage/country_create?id=${this.entity.id}`);
                            }
                        }
                    ]
                }
...
<template>
    <div style="text-align: right" :data-testid="actionId" :id="actionId">
        ...
        <ConfirmModal :country="entity" :close="DeleteConfirmDialog" :display="displayConfirm"> </ConfirmModal>
    </div>
</template>
