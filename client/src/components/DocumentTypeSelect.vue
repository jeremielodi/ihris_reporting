...
import DocumentTypeService from '@/views/pages/manage/document_type/document_type.service';

export default defineComponent({
    name: 'DocumentTypeServiceSelect',
    components: { Dropdown },
    props: {
    ...
    watch: {
        ...
        value(newVal) {
            if (newVal !== null && newVal !== undefined) {
                const result = this.documenttypes.filter((r) => r.id === newVal)[0];
    ...
    async mounted() {
        const result = await DocumentTypeService.read(null, {});
...
<template>
    <div class="field" style="margin-top:10px">
        ...
        <div class="col-12" style="padding: 0px">
            <Dropdown :id="id" v-model="dropdownValue" filter :options="documenttypes" optionLabel="name" style="width: 100%" placeholder="Select" @change="onSelect" :class="{ 'p-invalid': showInvalidMsg }" />
        </div>
