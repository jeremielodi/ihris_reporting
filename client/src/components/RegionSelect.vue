...
import RegionService from '@/views/pages/manage/region/region.service';

export default defineComponent({
    name: 'RegionServiceSelect',
    components: { Dropdown },
    props: {
    ...
    watch: {
        ...
        value(newVal) {
            if (newVal !== null && newVal !== undefined) {
                const result = this.regions.filter((r) => r.id === newVal)[0];
    ...
    async mounted() {
        const result = await RegionService.read(null, {});
        this.regions = result || [];
    },
    methods: {
        onSelect(e) {
...
<template>
    <div class="field">
        ...
        <div class="col-12" style="padding: 0px">
            <Dropdown :id="id" v-model="dropdownValue" filter :options="regions" optionLabel="name" style="width: 100%" placeholder="Select" @change="onSelect" :class="{ 'p-invalid': showInvalidMsg }" />
        </div>
