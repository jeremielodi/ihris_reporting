...
import countrieservice from '@/views/pages/manage/country/country.service';

export default defineComponent({
    name: 'countrieserviceSelect',
    components: { Dropdown },
    props: {
    ...
    watch: {
        ...
        value(newVal) {
            if (newVal !== null && newVal !== undefined) {
                const result = this.countries.filter((r) => r.id === newVal)[0];
    ...
    async mounted() {
        const result = await countrieservice.read(null, {});
...
<template>
    <div class="field" style="margin-top: 10px;">
        ...
        <div class="col-12" style="padding: 0px">
            <Dropdown :id="id" v-model="dropdownValue" filter :options="countries" optionLabel="name" style="width: 100%" placeholder="Select" @change="onSelect" :class="{ 'p-invalid': showInvalidMsg }" />
        </div>
