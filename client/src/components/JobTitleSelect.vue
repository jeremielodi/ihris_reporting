...
import Job_titleService from '@/views/pages/manage/job_title/job_title.service';

export default defineComponent({
    name: 'Job_titleServiceSelect',
    components: { Dropdown },
    props: {
    ...
    watch: {
        ...
        value(newVal) {
            if (newVal !== null && newVal !== undefined) {
                const result = this.job_titles.filter((r) => r.id === newVal)[0];
    ...
    async mounted() {
        const result = await Job_titleService.read(null, {});
        this.job_titles = result || [];
    },
    methods: {
        onSelect(e) {
...
<template>
    <div class="field" style="margin-top: 10px;">
        ...
        <div class="col-12" style="padding: 0px">
            <Dropdown :id="id" v-model="dropdownValue" filter :options="job_titles" optionLabel="name" style="width: 100%" placeholder="Select" @change="onSelect" :class="{ 'p-invalid': showInvalidMsg }" />
        </div>
