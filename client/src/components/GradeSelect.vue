...
import GradeService from '@/views/pages/manage/grade/grade.service';

export default defineComponent({
    name: 'GradeServiceSelect',
    components: { Dropdown },
    props: {
    ...
    async mounted() {
        const result = await GradeService.read(null, {});
        this.grades = result || [];
        ...
        setValue(newVal) {
            if (newVal !== null && newVal !== undefined) {
                if (typeof newVal == 'object') {
                    const result = this.grades.filter((r) => r.id === newVal.id)[0];
                    this.dropdownValue = result;
                } else {
                    const result = this.grades.filter((r) => r.id === newVal)[0];
                    this.dropdownValue = result;
                }
            } else {
...
<template>
    <div class="field">
        ...
        <div class="col-12" style="padding: 0px">
            <Dropdown :id="id" v-model="dropdownValue" filter :options="grades" optionLabel="name" style="width: 100%" placeholder="Select" @change="onSelect" :class="{ 'p-invalid': showInvalidMsg }" />
        </div>
