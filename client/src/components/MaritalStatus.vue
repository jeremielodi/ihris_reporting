...
import Gender from '@/views/pages/manage/marital_status/marital_status.service';
        ...
        setValue(newVal) {
            if (newVal !== null && newVal !== undefined) {
                if (typeof newVal == 'object') {
                    const result = this.status.filter((r) => r.id === newVal.id)[0];
                    this.dropdownValue = result;
                } else {
                    const result = this.status.filter((r) => r.id === newVal)[0];
                    this.dropdownValue = result;
                }
            } else {
...
<template>
    <div class="field" style="margin-top: 10px;">
        ...
        <div class="col-12" style="padding: 0px">
            <Dropdown :id="id" v-model="dropdownValue" :options="status" optionLabel="name" style="width: 100%" placeholder="Select" @change="onSelect" :class="{ 'p-invalid': showInvalidMsg }" />
        </div>
