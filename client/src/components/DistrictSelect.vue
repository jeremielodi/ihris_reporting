...
import DistrictService from '@/views/pages/manage/district/district.service';

export default defineComponent({
    name: 'DistrictServiceSelect',
    components: { Dropdown },
    props: {
    ...
    watch: {
        ...
        value(newVal) {
            if (newVal !== null && newVal !== undefined) {
                 console.log(newVal);
                const result = this.districts.filter((r) => r.id === newVal)[0];
    ...
    async mounted() {
        const result = await DistrictService.read(null, {});
...
<template>
    <div class="field" style="margin-top: 10px;">
        ...
        <div class="col-12" style="padding: 0px">
            <Dropdown :id="id" v-model="dropdownValue" filter :options="districts" optionLabel="name" style="width: 100%" placeholder="Select" @change="onSelect" :class="{ 'p-invalid': showInvalidMsg }" />
        </div>
