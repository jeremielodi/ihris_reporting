...
import data from '../../tests/mock/cadre.json';
...
export default function cadre() {
    test.describe('cadre', () => {
        test('invalid empty form shows invalid message', async ({ page }) => {
            ...
            await page.goto('/#/manage/cadre_registry');
        ...
        test('shows validation only after submit', async ({ page }) => {
            ...
            await page.fill('#name', data.cadre.name);
            await page.fill('#code', data.cadre.code);
            await page.fill('#description', data.cadre.description);
        ...
        test('shows edit cadre', async ({ page }) => {
            await login(page);
            await sleep(1000);
            ...
            await page.getByTestId('cadreAction1').click();
            await page.getByText('Editer').click();

            await sleep(1000);

            await page.fill('#description', data.cadre_update.description);
            await page.getByTestId('submitButton').click();
        ...
        test('should import cadres', async ({ page }) => {
            await login(page);
            await sleep(1000);
            ...
            fileContent += data.cadre_bulk.map((cls) => `${cls.code},${cls.name},${cls.description}`).join('\n');

            const filePath = 'temp_cadre_import.csv';
